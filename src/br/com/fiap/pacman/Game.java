...
public class Game extends JFrame implements KeyListener {
	...
	private Ghost ghost1 = new Ghost(100,500,0);
	private Ghost ghost2 = new Ghost(500,500,0);
	private Ghost ghost3 = new Ghost(100,500,0);
	private Ghost ghost4 = new Ghost(500,500,0);
	private Bomb bomb = new Bomb(150,150);
	private Booster booster = new Booster(400, 400, 15);
		...
		while (player.getLife() > 0) {
			...
			if(bomb.getX() == player.getX() && bomb.getY() == player.getY()){
				if(!player.isInvencivel())
					bomb.setTiraVidas(player);
				
			}
				
						if(ghost1.getX() == player.getX() && ghost1.getY() == player.getY()){
							if(!player.isInvencivel())
								player.setLife((int) (player.getLife() - 5));
						}
						if(ghost2.getX() == player.getX() && ghost2.getY() == player.getY()){
							if(!player.isInvencivel())
								player.setLife((int) (player.getLife() - 5));
						}
						if(ghost3.getX() == player.getX() && ghost3.getY() == player.getY()){
							if(!player.isInvencivel())
								player.setLife((int) (player.getLife() - 5));
						}
						if(ghost4.getX() == player.getX() && ghost4.getY() == player.getY()){
							if(!player.isInvencivel())
								player.setLife((int) (player.getLife() - 5));
						}
